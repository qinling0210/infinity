...
def generate(generate_if_exists: bool, copy_dir: str):
    ...
    with open(import_slt_path, "w") as slt_file:
            ...
            for row_id in range(row_n):
                ...
                for i in range(len(data[row_id])):
                    ...
                    for j in range(len(data[row_id][i])):
                        slt_file.write("[")
                        for k in range(dim):
                            slt_file.write(f"{data[row_id][i][j][k]}")
                            if k != dim - 1:
                                slt_file.write(",")
                        slt_file.write("]")
        ...
        slt_file.write("statement ok\n")
        slt_file.write(
            f"CREATE TABLE {table_name} (c1 INT, c2 TENSORARRAY(INT, {dim}));\n"
        )
        slt_file.write("\n")
        ...
        slt_file.write("statement ok\n")
        slt_file.write(
            f"CREATE TABLE {table_name1} (c1 INT, c2 TENSORARRAY(INT, {dim}));\n"
        )
        slt_file.write("\n")
        ...
        slt_file.write("statement ok\n")
        slt_file.write(
            f"CREATE TABLE {table_name_err} (c1 INT, c2 TENSORARRAY(INT, {dim + 1}));\n"
        )
        slt_file.write("\n")
